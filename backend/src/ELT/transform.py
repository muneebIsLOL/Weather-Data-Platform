...
def clean_data(data: dict):
    ...
    metadata = data["weather_metadata"]
    ...
    metadata["utc_offset_secs"] = metadata["utc_offset_secs"].fillna(
        metadata["fetched_at"].dt.utcoffset().dt.total_seconds()
    )
...
def column_mapping(data: dict):
    ...
    for conditions in [current_conditions, hourly_conditions]:
        conditions["feels_like"] = (
            conditions["weather_code"].map(WEATHER_CODES).fillna("NS").astype("category")
        )
...
def normalize(data: dict):
    for value in [data["current_conditions"], data["hourly_conditions"], data["daily_conditions"]]:
